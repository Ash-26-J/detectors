...
LOG_FILE = os.environ.get("UEBA_LOG", r"D:\test-siem\t1.json")
...
def _chrome_profile_dirs() -> dict:
    """
    Returns {browser_name: profile_dir} for every installed Chromium-based browser.
    Covers: Chrome, Brave, Edge, Opera, Vivaldi, Chromium, DuckDuckGo, Arc, Yandex.
    DuckDuckGo Privacy Browser (desktop) is Chromium-based and uses the same
    SQLite schema as Chrome, so it slots in here naturally.
    """
    ...
    if OS == "Linux":
        bases = {
            ...
            "duckduckgo": f"{home}/.config/DuckDuckGo/Browser",   # DDG desktop Linux
        ...
        bases = {
            ...
            "duckduckgo": f"{lib}/DuckDuckGo/Browser",            # DDG desktop macOS
        ...
        bases = {
            ...
            "chromium":   rf"{appdata}\Chromium\User Data",
            ...
            "vivaldi":    rf"{appdata}\Vivaldi\User Data",
            "duckduckgo": rf"{appdata}\DuckDuckGo\Browser\User Data",  # DDG desktop Windows
            "arc":        rf"{appdata}\Arc\User Data",
            "yandex":     rf"{appdata}\Yandex\YandexBrowser\User Data",
        }
    ...
    for browser, base in bases.items():
        if not os.path.isdir(base):
            continue
        # Opera stores its profile directly in the base dir; all others use Default/
...
def _ff_profile_dirs() -> list:
    """
    Returns a list of profile dirs for every Firefox-based browser.
    Covers: Firefox, LibreWolf, Waterfox, Floorp, Ghostery Dawn, IceCat.
    Each stores its history in places.sqlite with an identical schema.
    """
    home = os.path.expanduser("~")

    if OS == "Linux":
        # Each entry is (app_name, profiles_root)
        candidates = [
            # native installs
    ...
    for browser_name, root in candidates:
        ...
        for d in os.listdir(root):
            ...
            if os.path.exists(os.path.join(p, "places.sqlite")):
                seen.add(real)
                profiles.append((browser_name, p))   # now a (name, path) tuple
    return profiles
...
def _collect_visits_firefox(browser: str, profile_dir: str):
    ...
    if not os.path.exists(db_path):
        return
    tmp = _copy_db(db_path)
    # key by (browser, profile_dir) so multiple profiles don't share state
